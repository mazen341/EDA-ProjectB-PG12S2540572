...
submission = {
    ...
    "feature_engineering": {
        ...
        "baseline_features": [c for c in feature_cols if c.startswith("lag_")],
    ...
    "modeling_and_evaluation": {
        ...
        "feature_importance_table": importance_df.to_dict(orient="records"),
    ...
    "dashboard": {
        ...
        "has_simulator": True,
        "has_downloads": True,
        "graph_types": ["line", "bar", "scatter", "histogram", "heatmap", "flowchart", "technical diagrams"],
    },
    "presentation_and_rigor": {
        "limitations": [
            "Demo/live values are simulated unless connected to real SCADA.",
            "External image URLs should be replaced with local assets for offline deployment.",
        ],
        "reproducibility_notes": [
            "Run with streamlit run app.py",
            "Install requirements.txt",
            "Use chronological split for validation.",
        ],
    },
}
...
def local_grade(sub: dict[str, Any]) -> dict[str, Any]:
    scores = {
        ...
        "Modeling & evaluation": 22 if not comparison_df.empty else 14,
        "Dashboard quality": 10,
        "Presentation & rigor": 9,
    }
    return {
        "scores": scores,
        "total_80": int(sum(scores.values())),
        "strengths": [
            "Comprehensive interactive PV website",
            "Clear data pipeline and feature engineering",
            "Controlled model comparison and exports",
            "Live telemetry, technical diagrams, simulator, and transparent downloadable charts",
        ],
        "weaknesses": [
            "Live values are simulated unless connected to real telemetry.",
            "External image URLs should be replaced with local project photos for final offline deployment.",
        ],
        "actionable_improvements": [
            "Add a real SCADA/API connection.",
            "Replace external image URLs with local project photos.",
            "Add SHAP explainability if deployment allows the package.",
        ],
    }


# Sidebar export and grading tools stay accessible from every page.
with st.sidebar:
    st.markdown("---")
    ...
    with st.expander("Submission JSON + grading tools", expanded=True):
        openrouter_api_key = st.text_input(
            "OpenRouter API key",
            type="password",
            help="Optional. If empty, the app uses the local fallback grader.",
        ...
        show_submission_preview = st.checkbox(
            "Preview submission JSON",
            value=False,
            key="sidebar_preview_submission_json",
        )
        if show_submission_preview:
            st.json(submission)

        if st.button("🤖 Run AI grader / fallback", use_container_width=True, key="sidebar_run_ai_grader"):
            ...
            if openrouter_api_key:
                ...
                try:
                    ...
                    st.warning(f"AI grader unavailable or rate-limited; using local fallback. {exc}")
            else:
                st.info("No OpenRouter key provided, so the local fallback grader was used.")
        ...
        if grade_result:
            ...
            st.download_button(
                "⬇️ Download grade result",
                json.dumps(grade_result, indent=2, default=safe_json_default),
                "grade_result.json",
                "application/json",
                use_container_width=True,
                key="sidebar_download_grade_result",
            )

        st.markdown("#### Data exports")
        st.download_button(
            "⬇️ Cleaned dataset CSV",
            prepared_df.to_csv(index=False),
            "cleaned_dataset.csv",
            "text/csv",
            use_container_width=True,
            key="sidebar_download_cleaned",
        )
        st.download_button(
            "⬇️ Model features CSV",
            model_df.to_csv(index=False),
            "model_features.csv",
            "text/csv",
            use_container_width=True,
            key="sidebar_download_features",
        )
        if not comparison_df.empty:
            ...
            st.download_button(
                ...
                "text/csv",
                use_container_width=True,
                key="sidebar_download_importance",
            )
...
if selected_page == "🏠 Home":
    ...
    section_hero("📤 Export", "Export and AI grading tools are kept in the sidebar so they are accessible from every page.", "export")

    st.markdown(
        """
        <div class="panel">
            <div class="section-title">📤 Export and AI Grader are in the sidebar</div>
            <div class="muted">
                Use the sidebar panel named <b>Export and AI Grader</b> to preview the submission JSON,
                enter the OpenRouter API key, run the AI grader with 429-safe local fallback,
                and download submission, grade, cleaned data, feature matrix, metrics, predictions,
                and feature-importance files.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with c3:
        st.metric("Export files", "Sidebar")
